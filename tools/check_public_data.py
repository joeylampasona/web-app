...
--------------------------
Gamma. The rule is that the deepest few option books are free in full and
everything below them is gated, applied in both places gamma is published: the
board, and each stock page. So the public board must carry no more than the
free sample, and no stock file may carry a gamma payload for a name that is not
in that sample. The second is the one worth having: gating a ranking while
publishing every row of it gives the ranking away to anyone willing to fetch a
few hundred files.
...
from publish.gated import FREE_GAMMA_ROWS  # noqa: E402
...
def check(out: pathlib.Path) -> list[str]:
    ...
    for path in stocks:
        ...
        if payload.get("gamma_gated"):
            marked += 1
        if gamma and symbol not in free:
            leaked.append(symbol)

    print(f"  stocks/             {len(stocks)} file(s)   "
          f"{len(free)} carrying public gamma   {marked} marked gated")
    if leaked:
        shown = ", ".join(sorted(leaked)[:12])
        more = f" and {len(leaked) - 12} more" if len(leaked) > 12 else ""
        failures.append(
            f"{len(leaked)} stock file(s) publish gamma for names outside the "
            f"free sample ({shown}{more}). The board is gated and its rows are "
            f"not, which gives the board away to anyone who fetches the files.")
